...
namespace Relocation_Section_Editor
{
    public class PeHeader
    {
        ...
        {
            public UInt32 RelativeVirtualAddress;
            public UInt32 Size;
        }
        ...
        public PeHeader(string filePath)
        {
            ...
            using (FileStream stream = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                if (this.Is32BitImage && this.Is32BitMachine)
                {
                    ...
                    // Read directories
                    DataDirectories = new IMAGE_DATA_DIRECTORY[OptionalHeader32.NumberOfRvaAndSizes];
                ...
                else if (this.Is64BitMachine)
                {
                    ...
                    // Read directories
                    DataDirectories = new IMAGE_DATA_DIRECTORY[OptionalHeader64.NumberOfRvaAndSizes];
                    for (uint dirNo = 0; dirNo < DataDirectories.Length; dirNo++)
                    {
                        DataDirectories[dirNo] = FromBinaryReader<IMAGE_DATA_DIRECTORY>(reader);
                    }
        ...
        public IMAGE_SECTION_HEADER GetSectionInfo(UInt16 imgDir)
        {
            if (IsValidDirectory(imgDir))
            {
                for (uint headerNo = 0; headerNo < ImageSectionHeaders.Length; headerNo++)
                {
                    if ((DataDirectories[imgDir].RelativeVirtualAddress >= ImageSectionHeaders[headerNo].VirtualAddress) && (DataDirectories[imgDir].RelativeVirtualAddress < (ImageSectionHeaders[headerNo].VirtualAddress + ((ImageSectionHeaders[headerNo].VirtualSize > 0) ? ImageSectionHeaders[headerNo].VirtualSize : ImageSectionHeaders[headerNo].SizeOfRawData))))
                    {
        ...
        public bool Is32BitImage
        {
            get
            {
                return (FileHeader.Characteristics & IMAGE_FILE_32BIT_MACHINE) == IMAGE_FILE_32BIT_MACHINE;
            }
        }
        ...
        public bool Is32BitMachine
        {
            get
            {
                return (FileHeader.Machine & IMAGE_FILE_MACHINE_I386) == IMAGE_FILE_MACHINE_I386;
            }
        }
        ...
        public bool Is64BitMachine
        {
            get
            {
                return (FileHeader.Machine & IMAGE_FILE_MACHINE_AMD64) == IMAGE_FILE_MACHINE_AMD64;
            }
        }
